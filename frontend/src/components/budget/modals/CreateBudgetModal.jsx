...
export default function CreateBudgetModal({ period, onClose, onCreated }) {
  ...
  const save = async () => {
    ...
    // range & decimals check for main fields (defensive)
    ...
    const payload = {
      ...
      dtd: { amount: subBudgets.reduce((s, x) => s + x.amount, 0), subBudgets },
    };
        ...
        <div className="w-full max-w-5xl rounded-2xl bg-white shadow-2xl border border-slate-200">
          ...
          <div className="px-6 py-5 grid grid-cols-12 gap-4">
            ...
            <RightDTD
              ...
              form={form}
              setDTD={setDTD}
              dtdTotal={Object.values(form.dtd).reduce((s, v) => s + Number(v || 0), 0)}
              basisLabel={basisLabel}
              basisValue={basisValue}
              totalBudgeted={Number(form.savings || 0) + Number(form.commitments || 0) + Number(form.events || 0) + Object.values(form.dtd).reduce((s, v) => s + Number(v || 0), 0)}
              remaining={basisValue - (Number(form.savings || 0) + Number(form.commitments || 0) + Number(form.events || 0) + Object.values(form.dtd).reduce((s, v) => s + Number(v || 0), 0))}
              over={basisValue - (Number(form.savings || 0) + Number(form.commitments || 0) + Number(form.events || 0) + Object.values(form.dtd).reduce((s, v) => s + Number(v || 0), 0)) < 0}
              err={err}
          ...
          <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-end gap-2">
            ...
            <button
              disabled={saving || (basisValue - (Number(form.savings || 0) + Number(form.commitments || 0) + Number(form.events || 0) + Object.values(form.dtd).reduce((s, v) => s + Number(v || 0), 0)) < 0)}
              onClick={save}
              className={`px-4 py-2 rounded-xl text-white ${
                (basisValue - (Number(form.savings || 0) + Number(form.commitments || 0) + Number(form.events || 0) + Object.values(form.dtd).reduce((s, v) => s + Number(v || 0), 0)) < 0)
