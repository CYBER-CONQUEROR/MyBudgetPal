// src/components/budget/modals/EditOneModal.jsx
...
export default function EditOneModal({
  period,
  field,
  currentAmount,
  income,
  ...
  const [val, setVal] = useState(() => {
    const initial = Number(currentAmount ?? 0).toFixed(2);
  ...
  /* ===== derived sums using numeric value of input ===== */
  ...
  const nextTotal = numericVal + Number(othersSum || 0);
  const over = nextTotal > Number(income || 0);

  // 👇 max you’re allowed to type for this field so that projected total <= income
  const incomeCap = Math.max(0, Number(income || 0) - Number(othersSum || 0));
  ...
  const onAmountChange = (e) => {
    ...
    if (candidate > incomeCap) {
      const cappedStr = incomeCap.toFixed(Math.min(2, (String(incomeCap).split(".")[1] || "").length));
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      // ensure 2-decimal precision sent to API
      const amount = Number(Math.min(toNumber(val), incomeCap).toFixed(2));
      await patchPlan(period, { [field]: { amount, rollover: false, hardCap: false } });
      onSaved?.();
    } catch (e) {
        ...
        <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl border border-slate-200">
          ...
          <div className="px-6 py-3 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                className={`w-full rounded-xl border ${over ? "border-rose-300" : "border-slate-200"} pl-12 pr-3 py-2`}
                inputMode="decimal"
                value={val}
          ...
          <div className="px-6 py-3 border-t flex items-center justify-end gap-2">
            ...
            <button
              disabled={
                saving ||
                over ||
                !moneyRegex.test(cleanAmount(val)) ||
                toNumber(val) <= 0
              }
              onClick={save}
